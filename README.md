...
| 解除绑定 | 待验证 | |
| 扫码功能（商米 AIDL） | ✅ 已修复 | AIDL 接口修正、BaseActivity 全局拦截、官方 demo 规范绑定 |
| 美丰退货 | ✅ 已移植 | 退货列表/录入/详情，复用扫码+打印，见 5.16 |
...
| 54 | 架构 | BaseActivity 全局拦截所有物理按键 | 物理扫码键可能发送多种 keyCode（不限于 DPAD），只拦部分按键总会有漏 | `BaseActivity.dispatchKeyEvent` 拦截**全部**物理按键（只放行 BACK/VOLUME_UP/VOLUME_DOWN），物理扫码走硬件通路（激光→广播），不需要按键事件配合 |
| 55 | 扫码 | 按官方 demo 规范绑定服务 | 之前只 `bindService`，部分设备可能绑定不上 | 参考 `ScanHeadDemo`，先 `startService` 再 `bindService` |

#### ⚠️ 重要知识点：商米物理扫码键的处理方式

商米 V3PLUS 两侧的物理扫码按键按下后做两件事：
1. **硬件扫码**（激光亮）→ 扫到码后发广播
...
### 5.15 v2026-06-17 打印功能完整实现（v1.3.0）

**实现内容**：GoodlistActivity 接入商米 PrinterX SDK lineApi，实现出库单明细打印。

| # | 模块 | 改动 | 说明 |
|---|------|------|------|
| 56 | 打印 | PrinterX SDK 依赖 | `build.gradle` 添加 `com.sunmi:printerx:1.0.18` 依赖 |
| 57 | 打印 | 打印按钮布局 | `activity_goodlist.xml` 头部右侧添加「打印」按钮，状态为已出库/已退库时显示 |
| 58 | 打印 | 打印实现 | `GoodlistActivity.kt` 实现 `printOrder()`，使用 PrinterX SDK lineApi 直印，不用事务模式 |
| 59 | 打印 | 纸宽自适应 | 点打印弹出「请选择纸仓大小」对话框，手动选择 58mm/80mm，各自独立排版参数 |
| 60 | 打印 | 58mm 排版 | 列宽 `1:2:2:3:1`，明细字体 16px，列头 18px 加粗，行间距 4px，字符间距 1px |
| 61 | 打印 | 80mm 排版 | 列宽 `1:2:1:3:1`，明细字体 22px，列头 24px 加粗，表头 24px |
| 62 | 打印 | 底部留白 | 打印内容结束后留 80px（≈1cm）空白再走纸切纸 |
...
------ 分隔线 ------
序号  箱码  产品名称  规格  规格2
  1    CODE   名称    Spec  Spec2（58mm/80mm 各自独立排版）
  2    CODE   名称    Spec  Spec2
------ 分隔线 ------
           ← 1cm 空白
           ← 自动切纸
```
...
### 5.17 v2026-06-17 扫码优化：5秒熄光保护 + 连续扫码修复（v1.4.1）

**优化内容**：

| # | 模块 | 改动 说明 |
|---|------|------|
| 68 | 扫码（两页面） | 新增 5 秒熄光保护：扫码开始后 5 秒内无广播自动 `stopScan()` 恢复按钮状态 |
| 69 | 扫码（两页面） | 每次收到广播重置 5 秒超时计时器 |
| 70 | 美丰退货扫码 | 连续扫码逻辑从 `handleCodeResult` 成功分支移到 `ScanReceiver`，与新增出库一致，扫码结果无论成功/失败都保持激光常亮 |
| 71 | 美丰退货对话框 | 「继续扫描」按钮额外调 `scan()` 兜底 |
| 72 | 打印布局 | 58mm/80mm 列宽统一为 `4:24:16:1:24:1:8`（序号竖排/箱码放大/列间缝隙） |
...
### 5.24 v2026-06-22 打印列宽调整 + Toast 修复 + 登录页文案

| # | 模块 | 改动 |
|---|------|------|
| 89 | 出库单明细打印 | 列宽改为8列：序号(4)/箱码(18)/spacer/产品名(11)/spacer/规格(31)/spacer/规格2(11) |
| 90 | 出库单明细打印 | "正在打印" Toast 从选纸仓前移到选后才显示 |
| 91 | 登录页 | "PDA 管理端" → "PDA 客户端" |

**实现内容**：

- **列宽调整**：箱码列缩小到刚好两行，产品名称列固定4中文字符宽，箱码与产品名之间新增间隔列。省出的空间按 3:1 分给规格和规格2
- **Bug修复**：Toast "正在打印" 原来在弹出纸仓选择对话框前就执行了，用户还没选58/80就先看到提示。移到 `doPrint` 里，选完纸仓后真正打印时才显示
- **Bug修复**：列数从7→8后，`printTexts` 的文本/样式数组没同步改为8列，导致内容不打印。三处（表头"序"行、"号"行、明细行）补全8列对齐

**涉及文件**：
- 修改 `GoodlistActivity.kt`（列宽 + 8列对齐 + Toast 位置）
- 修改 `activity_login.xml`（文案修改）
...
**技术方案**：商米 PrinterX SDK lineApi（行式打印接口），详见 `doc/sunmi/商米接口开发文档.md` 和官方 Demo `SunmiPrinterXSample`

**纸仓选择**：点击打印弹出「请选择纸仓大小」对话框，手动选 58mm 或 80mm。
...
| 打印按钮显示条件 | **仅已出库、已退库状态显示**（此时出库/删除按钮已隐藏）。`status == "已收货" \|\| status == "已退库"` |
| 打印内容 | 表头（单据编号、客户代码、客户名称、日期、类型）+ 列名行（序号、箱码、产品名称、规格、规格2）+ 明细列表 |
| 排版布局 | 58mm 列宽 `1:2:2:3:1`(14/16/18px)，80mm 列宽 `1:2:1:3:1`(22/24px) |
| 纸宽适配 | 手动选择 58mm/80mm，各自独立排版参数，自适应像素宽度 |

### <3> 美丰退货 ✅ 已实现

**页面**：菜单页 → 美丰退货（含退货列表/退货录入/退货详情 3 个页面）

**技术方案**：复用商米扫码头引擎 AIDL + lineApi 打印，后端 API `refundOrder/*`，后端 0 改动。

**实现细节**（v1.4.0 见 5.16）：

| 页面 | 说明 |
|------|------|
| 退货列表 | Tab 全部/待审核，查看/删除/复制条码，分页防竞态 |
| 退货录入 | 商米扫码录入 + 手动输入，保存退货单 |
| 退货详情 | 查看明细 + 复制全部条码 |

**Bug 修复**（v1.4.0）：
- Android 14 广播注册 `RECEIVER_EXPORTED` 标志
- 物理扫码键击穿提示框（focusable=false + onKeyListener 拦截）
- saveorder 参数 JSONArray 加引号
- 去除扫码时 showLoading 闪烁

### <4> 开发顺序

1. **先做扫码**：实现扫码头引擎接入 → 测试通过
2. **再做打印**：实现 lineApi 打印 → 测试通过
3. **再移植美丰退货**：移植小程序退货模块，复用扫码 → 测试通过
...
**✅ 第二步：商米 V3PLUS 硬件接入已完成**（2026-06-17）

1. ✅ **扫码功能已完整修复**：
   - AIDL 接口按官方文档修正，软件扫码按钮正常工作
   - `BaseActivity` 全局拦截物理按键，两侧物理扫码键不会触发导航跳转
   - 服务绑定按官方 demo 规范：先 `startService` 再 `bindService`
   - 连续模式扫描，广播输出，重复码/无效码弹框正常
2. ✅ **打印功能已实现**：
   - GoodlistActivity 接入 PrinterX SDK lineApi，58mm/80mm 自适应
   - 手动选择纸仓大小，各自独立排版参数
3. ✅ **美丰退货已移植**：
   - 退货列表/录入/详情 3 页面完整移植
   - 复用商米扫码 + 后端 0 改动
